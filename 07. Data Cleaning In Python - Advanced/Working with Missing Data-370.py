...
plot_null_correlations(mvc[[col for col in mvc.columns if 'vehicle'  in col]])
...
for v in range(1,6):
    ...
    vc_null_data.append([v, (mvc[v_col].isnull() & ~mvc[c_col].isnull()).sum(), (~mvc[v_col].isnull() & mvc[c_col].isnull()).sum()])
...
for col in location_cols:
    the_mask = mvc[col].isnull()
    mvc[col] = mvc[col].mask(the_mask, sup_data[col])
